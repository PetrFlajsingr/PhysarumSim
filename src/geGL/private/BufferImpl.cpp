...
GLint BufferImpl::getBufferParameter(GLenum pname) const {
  GLint param;
  glGetNamedBufferParameteriv(buffer->getId(), pname, &param);
  return param;
}

GLint64 BufferImpl::getBufferParameter64(GLenum pname) const {
  GLint64 param;
  glGetNamedBufferParameteri64v(buffer->getId(), pname, &param);
  return param;
}

GLvoid *BufferImpl::getBufferPointer(GLenum pname) const {
  GLvoid *param;
  glGetNamedBufferPointerv(buffer->getId(), pname, &param);
  return param;
}
