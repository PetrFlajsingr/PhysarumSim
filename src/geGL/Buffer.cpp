...
void Buffer::copy(Buffer const &buffer) const {
  ...
  glCopyNamedBufferSubData(buffer.getId(), getId(), 0, 0, maxSize);
}
...
void Buffer::flushMapped(GLsizeiptr size, GLintptr offset) const {
  auto s = size;
  if (s == 0) s = getSize();
  glFlushMappedNamedBufferRange(getId(), offset, s);
}
...
GLvoid *Buffer::map(GLbitfield access) const {
  ...
  return glMapNamedBufferRange(getId(), 0, getSize(), a);
}
...
GLvoid *Buffer::map(GLintptr offset, GLsizeiptr size, GLbitfield access) const {
  return glMapNamedBufferRange(getId(), offset, size, access);
}

/**
 * @brief unmaps buffer
 */
void Buffer::unmap() const { glUnmapNamedBuffer(getId()); }
