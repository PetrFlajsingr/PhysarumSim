...
void Texture::create(
  ...
  glCreateTextures(this->_target, 1, &this->getId());
  if (levels > 0) {
    if (height == 0) glTextureStorage1D(this->getId(), levels, this->_format, width);
    else if (depth == 0)
      glTextureStorage2D(this->getId(), levels, this->_format, width, height);
    else
      glTextureStorage3D(this->getId(), levels, this->_format, width, height, depth);
  } else {
...
void Texture::bind(GLuint unit) const {
  assert(this != nullptr);
  glBindTextureUnit(unit, this->getId());
}
...
void Texture::setData2D(
  ...
  if (target != 0) {
    glTextureSubImage2DEXT(this->getId(), target, level, xoffset, yoffset, width, height, format, type, data);
  } else {
    glTextureSubImage2DEXT(this->getId(), this->_target, level, xoffset, yoffset, width, height, format, type, data);
  }
...
void Texture::getCompressedData(void *data, GLsizei bufSize, GLint level) {
  glGetCompressedTextureImage(getId(), level, bufSize, data);
}
...
void Texture::texParameteri(GLenum pname, GLint params) const {
  assert(this != nullptr);
  glTextureParameteri(this->getId(), pname, params);
}
...
void Texture::texParameterfv(GLenum pname, GLfloat *params) const {
  assert(this != nullptr);
#ifndef USE_DSA
  glBindTexture(this->getTarget(), this->getId());
  glTexParameterfv(this->getTarget(), pname, params);
#else //USE_DSA
  glTextureParameterfv(this->getId(), pname, params);
#endif//USE_DSA
}
...
GLuint Texture::_bindSafe() const {
  ...
  glGetIntegerv(textureTarget2Binding(target), (GLint *) &oldId);
  glBindTexture(target, this->getId());
  return oldId;
}
...
#else //USE_DSA
  glGetTextureLevelParameteriv(this->getId(), level, pname, &param);
#endif//USE_DSA
  return param;
}
...
GLint Texture::_getTexParameter(GLenum pname) const {
  ...
  glBindTexture(this->getTarget(), oldId);
#else //USE_DSA
  glGetTextureParameteriv(this->getId(),pname,&param));
#endif//USE_DSA
  return param;
}
...
void Texture::_getTexParameterf(GLfloat *data, GLenum pname) const {
  ...
  glBindTexture(this->getTarget(), oldId);
#else
  glGetTextureParameterfv(this->getId(), pname, data);
#endif//USE_DSA
}
...
std::string Texture::getInfo() const {
  ...
  ss << "GL_TEXTURE_INTERNAL_FORMAT: " << translateInternalFormat(this->getInternalFormat(0)) << std::endl;
  ...
  ss << "GL_TEXTURE_RED_TYPE: " << translateTextureChannelType(this->getRedType(0)) << std::endl;
  ss << "GL_TEXTURE_BLUE_TYPE: " << translateTextureChannelType(this->getBlueType(0)) << std::endl;
  ss << "GL_TEXTURE_GREEN_TYPE: " << translateTextureChannelType(this->getGreenType(0)) << std::endl;
  ss << "GL_TEXTURE_ALPHA_TYPE: " << translateTextureChannelType(this->getAlphaType(0)) << std::endl;
  ss << "GL_TEXTURE_DEPTH_TYPE: " << translateTextureChannelType(this->getDepthType(0)) << std::endl;
  ...
  ss << "GL_TEXTURE_SWIZZLE_R: " << translateTextureSwizzle(this->getSwizzleR()) << std::endl;
  ss << "GL_TEXTURE_SWIZZLE_G: " << translateTextureSwizzle(this->getSwizzleG()) << std::endl;
  ss << "GL_TEXTURE_SWIZZLE_B: " << translateTextureSwizzle(this->getSwizzleB()) << std::endl;
  ...
  ss << "GL_TEXTURE_WRAP_T: " << translateTextureWrap(this->getWrapT()) << std::endl;
  ss << "GL_TEXTURE_WRAP_R: " << translateTextureWrap(this->getWrapR()) << std::endl;
  ss << "GL_TEXTURE_TARGET: " << translateTextureTarget(this->getTarget()) << std::endl;
