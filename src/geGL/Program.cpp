...
GLint Program::_getParam(GLenum pname) const {
  assert(this != nullptr);
  GLint params;
  glGetProgramiv(getId(), pname, &params);
  return params;
}
...
GLint Program::getAttribLocation(std::string const &name) const {
  assert(this != nullptr);
  return glGetAttribLocation(getId(), name.c_str());
}
...
void Program::getComputeWorkGroupSize(GLint *x) const {
  assert(this != nullptr);
  glGetProgramiv(getId(), GL_COMPUTE_WORK_GROUP_SIZE, x);
}
...
std::string Program::getInfoLog() const {
  ...
  glGetProgramInfoLog(getId(), length, NULL, (GLchar *) info.c_str());
  return info;
}
...
void Program::_fillAttribInfo() {
  ...
  for (GLint i = 0; i < nofAttribs; ++i) {
    ...
    GLint location;
    GLsizei length;
    glGetActiveAttrib(getId(), i, longestAttrib, &length, &size, &type, buffer);
    name = _chopIndexingInPropertyName(std::string(buffer));
