...
GLint Framebuffer::getParam(GLenum pname) {
  assert(this != nullptr);
  GLint param;
  glGetNamedFramebufferParameteriv(this->getId(), pname, &param);
  return param;
}

void Framebuffer::setParam(GLenum pname, GLint param) {
  assert(this != nullptr);
  glNamedFramebufferParameteri(this->getId(), pname, param);
}

GLint Framebuffer::getAttachmentParam(GLenum attachment, GLenum pname) {
  assert(this != nullptr);
  GLint param;
  glGetNamedFramebufferAttachmentParameteriv(this->getId(), attachment, pname, &param);
  return param;
}
...
Framebuffer::Framebuffer(
  ...
  if (defaultFramebuffer) this->getId() = 0;
    ...
    glCreateFramebuffers(1, &this->getId());
}
...
void Framebuffer::attachRenderbuffer(
  ...
  if (!renderbuffer) {
    glNamedFramebufferRenderbuffer(this->getId(), attachment, GL_RENDERBUFFER, 0);
    return;
  }
...
void Framebuffer::bind(GLenum target) const {
  assert(this != nullptr);
  glBindFramebuffer(target, this->getId());
}
...
bool Framebuffer::check() const {
  assert(this != nullptr);
  return glCheckNamedFramebufferStatus(this->getId(), GL_DRAW_FRAMEBUFFER) == GL_FRAMEBUFFER_COMPLETE;
}

void Framebuffer::drawBuffer(GLenum buffer) const {
  assert(this != nullptr);
  glNamedFramebufferDrawBuffer(this->getId(), buffer);
}

void Framebuffer::drawBuffers(GLsizei n, const GLenum *buffers) const {
  assert(this != nullptr);
  glNamedFramebufferDrawBuffers(this->getId(), n, buffers);
}

void Framebuffer::drawBuffers(std::vector<GLenum> const buffers) const {
  assert(this != nullptr);
  glNamedFramebufferDrawBuffers(this->getId(), (GLsizei) buffers.size(), buffers.data());
}
...
void Framebuffer::clearBuffer(GLenum buffer, GLint drawBuffer, const GLint *value) const {
  assert(this != nullptr);
  glClearNamedFramebufferiv(this->getId(), buffer, drawBuffer, value);
}

void Framebuffer::clearBuffer(GLenum buffer, GLint drawBuffer, const GLfloat *value) const {
  assert(this != nullptr);
  glClearNamedFramebufferfv(this->getId(), buffer, drawBuffer, value);
}

void Framebuffer::clearBuffer(GLenum buffer, GLint drawBuffer, const GLuint *value) const {
  assert(this != nullptr);
  glClearNamedFramebufferuiv(this->getId(), buffer, drawBuffer, value);
}
...
GLboolean Framebuffer::isFramebuffer() const {
  assert(this != nullptr);
  return glIsFramebuffer(this->getId());
}
