...
pf::ogl::PhysarumRenderer::PhysarumRenderer(const std::filesystem::path &shaderDir,
                                            std::shared_ptr<Texture> trailTexture, glm::ivec2 renderResolution)
    : shaderDir(shaderDir), trailTexture(std::move(trailTexture)), renderResolution(renderResolution) {
  const auto renderTextureSrc = readFile(shaderDir / "physarum_render.comp");
  if (!renderTextureSrc.has_value()) { throw std::runtime_error("Could not load 'physarum_render.comp'"); }
...
void pf::ogl::PhysarumRenderer::init(const std::vector<physarum::PopulationColor> &populations) {
  ...
  for (std::size_t i = 0; i < populations.size(); ++i) { setConfig(populations[i], i); }
...
void pf::ogl::PhysarumRenderer::setColorLUT(const std::array<glm::vec3, 256> &lut, std::size_t index) {
  ...
  RAII unmap{[&] { colorLUTBuffer->unmap(); }};

  const auto bufferVec4Span =
      std::span{reinterpret_cast<glm::vec4 *>(bufferData), colorLUTBuffer->getSize() / sizeof(glm::vec4)};
  for (std::size_t i = index * 256; i < index * 256 + 256; ++i) { bufferVec4Span[i] = glm::vec4{lut[i % 256], 1.f}; }
}

const std::shared_ptr<Texture> &pf::ogl::PhysarumRenderer::getRenderTexture() const { return renderTexture; }
...
void pf::ogl::PhysarumRenderer::setConfig(const pf::physarum::PopulationColor &config, std::size_t index) {
  ...
  RAII unmap{[&] { speciesSettingsBuffer->unmap(); }};

  const auto bufferSpan = std::span{reinterpret_cast<details::SpeciesShaderRenderSettings *>(bufferData),
                                    colorLUTBuffer->getSize() / sizeof(details::SpeciesShaderRenderSettings)};
  bufferSpan[index] = settings;
  setColorLUT(config.getColorLUT(), index);
}
void pf::ogl::PhysarumRenderer::setBlendType(pf::ogl::BlendType blendType) { PhysarumRenderer::blendType = blendType; }
